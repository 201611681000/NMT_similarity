...
class FairseqTask(object):
    ...
    def train_step(self, sample, model, criterion, optimizer, train_step, ignore_grad=False):
        """
        ...
        model.train()
        #model.eval()
        loss, sample_size, logging_output = criterion(model, sample, train_step)
        if ignore_grad:
            loss *= 0
    ...
    def valid_step(self, sample, model, criterion, train_step):
        model.eval()
        with torch.no_grad():
            loss, sample_size, logging_output = criterion(model, sample, train_step)
        return loss, sample_size, logging_output
