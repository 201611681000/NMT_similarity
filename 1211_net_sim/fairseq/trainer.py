...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, dummy_batch=None, oom_batch=None):
        ...
        #self.generator = task.build_generator([model], args)
        self.generator = task.build_generator(args)
        self.mose_en = MosesDetokenizer(lang='en')
        self.mose_de = MosesDetokenizer(lang='de')
        if args.bert_model_path :
            self.bert_tokenizer = BertTokenizer.from_pretrained(args.bert_model_path)
            self.bert_model = BertModel.from_pretrained(args.bert_model_path)
            #print (device)
            #self.bert_tokenizer.cuda()
            self.bert_model.cuda()
            self.bert_model.eval()

            self.bert_de_token = BertTokenizer.from_pretrained(args.bert_german_path)
            self.bert_de_model = BertModel.from_pretrained(args.bert_german_path)
            self.bert_de_model.cuda()
            self.bert_de_model.eval()

        #self.FFN = FFN.FFNet(args.bert_hidden_size, args.net_hidden_size, 1).cuda()
    ...
    def init_meters(self, args):
        ...
        self.meters["train_sim_loss"] = AverageMeter()
        self.meters["valid_loss"] = AverageMeter()
        self.meters["valid_nll_loss"] = AverageMeter()
        self.meters["valid_sim_loss"] = AverageMeter()
    ...
    def generate_cand_translations (self, sample):
        ...
        for i, sample_id in enumerate(sample['id'].tolist()):
            ...
            for j in range (self.args.cands_into_model):
                ...
                smooth_bleuscore = sacrebleu.corpus_bleu([hypo_str],[[target_str]],use_effective_order=True,
                                                  tokenize = "none")
        ...
        with torch.no_grad():
            ...
            '''
            src_bert_ids, src_bert = get_bert_out (src_str_for_bert, self.bert_de_token, self.bert_de_model)
            cand_bert_ids, cand_bert = get_bert_out (cand_str_for_bert, self.bert_tokenizer, self.bert_model)
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                with maybe_no_sync():
                    sample = self.generate_cand_translations(sample)
                    # forward and backward
                    self.model.train()
                    torch.cuda.empty_cache()
                    loss, sample_size, logging_output = self.task.train_step(
                        sample, self.model, self.criterion, self.optimizer, True, ignore_grad
                    )
                    del loss
                    torch.cuda.empty_cache()
        ...
        with torch.no_grad():
            ...
            try:
                ...
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, self.criterion, False
                )
            except RuntimeError as e:
                if "out of memory" in str(e):
                    self._log_oom(e)
