    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        for sample in t:
            ...
            if random_list:
                ...
                sample['ntokens'] = torch.LongTensor([s.ne(2).long().sum() for s in sample['target']]).sum().item()
            ...
            for i, sample_id in enumerate(sample['id'].tolist()):
                ...
                decoded_out = get_decoded_out (current_src_tokens, current_src_length,
                                               temp_cand_tokens, models)
                #print (decoded_out.shape,"decoded")
                #get the encoded_out of bert
                encoded_input = bert_tokenizer(detoken_cands, return_tensors='pt',padding=True)
                ...
                net_out = models[0].decoder.through_ffnet(torch.cat((encoded_out, decoded_out),1)) #[beam, seq_len, 1]
                #print ("net_out", net_out.shape)
                sent_logits = torch.nn.functional.softmax(
                    net_out.view(-1,args.nbest),dim=1)
                #print (sent_logits)
                ...
                print ("-----net choice: {} bleu:{:.3f} pos: {}".format(
                        cands[net_pos], bleu_score[net_pos],net_pos+1),file=sys.stdout)
